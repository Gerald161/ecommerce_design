...
const AdminProductUpload = () => {
  ...
  const variantTypes = [
    { value: 'color-size', label: 'Color + Size', description: 'Best for clothing, shoes, etc.' },
    { value: 'color-only', label: 'Color Only', description: 'Best for toys, electronics, etc.' },
    { value: 'size-only', label: 'Size Only', description: 'Best for generic items, detergents, etc.' }
  ];
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Reset variant type when hasVariants is unchecked
        ...
        {formData.hasVariants && (
          <div className="w-full flex flex-col items-center">
            ...
            {formData.variantType && (
              <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1 max-w-md text-center`}>
                {variantTypes.find(t => t.value === formData.variantType)?.description}
              </p>
            )}
          </div>
        )}
  ...
  const MediaUploadStep = () => {
    const getVariantLabel = (variant) => {
      const { variantType } = formData;
      if (variantType === 'color-size') {
        return `${variant.color || 'No Color'} - ${variant.size || 'No Size'}`;
      } else if (variantType === 'color-only') {
        return variant.color || 'No Color';
      } else if (variantType === 'size-only') {
        return variant.size || 'No Size';
      }
      return 'Variant';
    };
    ...
    const renderVariantFields = (variant, index) => {
      ...
      return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {/* Color Selection - only show for color-size and color-only */}
          {(variantType === 'color-size' || variantType === 'color-only') && (
            <div>
          ...
          {/* Size Selection - only show for color-size and size-only */}
          {(variantType === 'color-size' || variantType === 'size-only') && (
            <div>
              <label className="flex items-center text-sm font-medium mb-2">
          ...
          {/* Stock - always show for variants */}
          <div>
            <label className="flex items-center text-sm font-medium mb-2">
              <Package className="w-4 h-4 mr-2" />
              Stock Quantity
      ...
      <div className="space-y-6">
        ...
        <div className={`p-6 rounded-lg border ${cardClasses} mb-8 max-w-4xl mx-auto`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-center sm:text-left">
            ...
            {formData.hasVariants && (
              <div><span className="font-medium">Variant Type:</span> {variantTypes.find(t => t.value === formData.variantType)?.label}</div>
            )}
            {!formData.hasVariants && formData.stock && (
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-semibold flex items-center">
                  ...
                  Product Variants ({variantTypes.find(t => t.value === formData.variantType)?.label})
                </h3>
                <button
                  type="button"
                  onClick={addVariant}
                ...
                <div key={variant.id} className={`p-6 rounded-lg border ${cardClasses}`}>
                  ...
                  <div className={`p-6 rounded-lg border-2 border-dashed transition-colors ${
                    ...
                    <div className="text-center">
                      ...
                      <h4 className="font-medium mb-2">Images for {getVariantLabel(variant)}</h4>
